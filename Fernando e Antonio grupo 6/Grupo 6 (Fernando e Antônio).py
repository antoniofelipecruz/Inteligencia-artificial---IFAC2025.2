import random
from collections import deque
import time
import os

# --- Configurações do Jogo ---
GRID_SIZE = 15
MAZE_DENSITY = 0.3 # 30% de chance de uma célula ser parede
DELAI_PASSO = 0.5 # Tempo de espera em segundos entre cada passo

# --- Símbolos do Labirinto ---
PATH = 0  # Caminho Livre (célula vazia no código)
WALL = 1  # Parede
PLAYER = 'J'
TREASURE = 'T'
ENEMY = 'E'
VISITED_PATH = '.' # Caminho percorrido pelo jogador
SHORTEST_PATH = '*' # O caminho mais curto (opcional na visualização)

# --- Funções de Ajuda ---

def clear_screen():
    """Limpa a tela do console para a animação."""
    os.system('cls' if os.name == 'nt' else 'clear')

def generate_maze(size, density):
    """Gera um labirinto aleatório e posiciona J, T e E."""
    
    # 1. Gera a grade base
    maze = [[PATH] * size for _ in range(size)]
    
    # 2. Adiciona as paredes
    for r in range(size):
        for c in range(size):
            # Garante bordas de parede e distribui paredes aleatórias no interior
            if r == 0 or r == size - 1 or c == 0 or c == size - 1 or random.random() < density:
                maze[r][c] = WALL

    # 3. Encontra posições válidas para J, T e E (caminho livre)
    free_cells = [(r, c) for r in range(size) for c in range(size) if maze[r][c] == PATH]
    
    if len(free_cells) < 3:
        # Se não houver células livres suficientes, tenta gerar novamente ou retorna erro
        return None, None, None, None

    # Seleciona 3 posições únicas aleatórias
    random.shuffle(free_cells)
    
    # Posições de Player (J), Treasure (T) e Enemy (E)
    pos_player = free_cells.pop()
    pos_treasure = free_cells.pop()
    pos_enemy = free_cells.pop()
    
    # Atualiza o labirinto
    r_j, c_j = pos_player
    r_t, c_t = pos_treasure
    r_e, c_e = pos_enemy
    
    maze[r_j][c_j] = PLAYER
    maze[r_t][c_t] = TREASURE
    maze[r_e][c_e] = ENEMY
    
    return maze, pos_player, pos_treasure, pos_enemy

def print_maze(maze, step, status):
    """Imprime o labirinto no console."""
    clear_screen()
    
    size = len(maze)
    print("--- CAÇA AO TESOURO BFS ---")
    print(f"Passo: {step}")
    print(f"Status: {status}\n")
    
    for r in range(size):
        row_display = []
        for c in range(size):
            cell = maze[r][c]
            if cell == WALL:
                row_display.append('#') # Caractere para Parede
            elif cell == PLAYER:
                row_display.append('\033[92mJ\033[0m') # Verde para Jogador
            elif cell == TREASURE:
                row_display.append('\033[93mT\033[0m') # Amarelo para Tesouro
            elif cell == ENEMY:
                row_display.append('\033[91mE\033[0m') # Vermelho para Inimigo
            elif cell == VISITED_PATH:
                 row_display.append('.') # Caminho visitado (deixa claro que não é PATH)
            else:
                row_display.append(' ') # Caminho Livre
        print(' '.join(row_display))
    print("---------------------------\n")


def bfs_shortest_path(maze, start, end):
    """
    Implementação do BFS para encontrar o caminho mais curto.
    Retorna o caminho como uma lista de coordenadas [(r1, c1), (r2, c2), ...].
    """
    rows, cols = len(maze), len(maze[0])
    # Fila: armazena tuplas (posição, caminho_ate_aqui)
    queue = deque([(start, [start])])
    # Conjunto para rastrear células visitadas (evita loops e reexploração)
    visited = {start}
    
    # Movimentos possíveis (Cima, Baixo, Esquerda, Direita)
    directions = [(-1, 0), (1, 0), (0, -1), (0, 1)] # dr, dc

    while queue:
        (r, c), path = queue.popleft()

        # Se o destino for encontrado
        if (r, c) == end:
            return path[1:] # Retorna o caminho sem a posição inicial

        # Explora vizinhos
        for dr, dc in directions:
            nr, nc = r + dr, c + dc
            
            # Verifica se o vizinho é válido (dentro da grade)
            if 0 <= nr < rows and 0 <= nc < cols:
                next_pos = (nr, nc)
                cell_content = maze[nr][nc]
                
                # Verifica se o vizinho é um caminho livre (PATH=0) ou o destino (T/E)
                # O jogador pode andar sobre PATH (0) e deve procurar o TREASURE ('T')
                # O inimigo pode andar sobre PATH (0) e deve procurar o PLAYER ('J')
                
                is_valid_move = False
                if cell_content == PATH:
                    is_valid_move = True
                # A condição de parada (o alvo) também deve ser tratada como um movimento válido
                elif next_pos == end:
                    is_valid_move = True
                
                if is_valid_move and next_pos not in visited:
                    visited.add(next_pos)
                    new_path = path + [next_pos]
                    queue.append((next_pos, new_path))
    
    return None # Nenhum caminho encontrado

# --- Lógica Principal do Jogo ---

def main_game_loop():
    # 1. Inicializa o Labirinto
    maze, pos_j, pos_t, pos_e = generate_maze(GRID_SIZE, MAZE_DENSITY)
    if not maze:
        print("Erro: Não foi possível gerar o labirinto com células livres suficientes.")
        return

    step_count = 0
    game_over = False
    
    # 2. Inicia o Loop do Jogo
    while not game_over:
        step_count += 1
        
        # --- Lógica do Jogador (J) ---
        
        # A. Encontra o caminho mais curto de J até T
        path_j_to_t = bfs_shortest_path(maze, pos_j, pos_t)

        if path_j_to_t:
            # Pega o próximo passo do caminho mais curto
            next_move_j = path_j_to_t[0]
            r_next_j, c_next_j = next_move_j
            
            # 1. Marca a posição atual do Jogador como Caminho Visitado
            r_j, c_j = pos_j
            if maze[r_j][c_j] == PLAYER:
                maze[r_j][c_j] = VISITED_PATH
            
            # 2. Move o Jogador
            pos_j = next_move_j
            r_j, c_j = pos_j

            # 3. Verifica Condição de Vitória
            if maze[r_j][c_j] == TREASURE:
                maze[r_j][c_j] = PLAYER # Posiciona J no T
                print_maze(maze, step_count, "\033[92mVITÓRIA!\033[0m O jogador encontrou o Tesouro!")
                game_over = True
                break
            
            # 4. Atualiza a nova posição no mapa (sempre move, a verificação é feita na função BFS)
            maze[r_j][c_j] = PLAYER
        
        else:
            # Se J não consegue mais encontrar T (está preso)
            print_maze(maze, step_count, "\033[93mFIM DE JOGO.\033[0m O jogador está preso e não consegue alcançar o Tesouro.")
            game_over = True
            break
            
        # --- Lógica do Inimigo (E) ---
        
        # B. Encontra o caminho mais curto de E até J (perseguição)
        # O inimigo persegue o JOGADOR na sua *nova* posição.
        path_e_to_j = bfs_shortest_path(maze, pos_e, pos_j)

        if path_e_to_j:
            # Pega o próximo passo do caminho mais curto para E
            next_move_e = path_e_to_j[0]
            r_next_e, c_next_e = next_move_e
            
            # 1. Marca a posição atual do Inimigo como Caminho Livre
            r_e, c_e = pos_e
            if maze[r_e][c_e] == ENEMY:
                maze[r_e][c_e] = PATH
            
            # 2. Move o Inimigo
            pos_e = next_move_e
            r_e, c_e = pos_e
            
            # 3. Verifica Condição de Derrota
            if pos_e == pos_j:
                # E chegou na posição de J (o que deve acontecer no próximo passo)
                maze[r_e][c_e] = ENEMY # Posiciona E em cima de J
                print_maze(maze, step_count, "\033[91mDERROTA!\033[0m O Inimigo atacou o Jogador.")
                game_over = True
                break
            
            # 4. Atualiza a nova posição no mapa (sempre move)
            maze[r_e][c_e] = ENEMY
            
        # 3. Imprime e Espera
        
        # Garante que o Tesouro (T) não seja sobrescrito pelo Path (0) caso J ou E o rodeie,
        # O tesouro sempre permanece visível até ser alcançado.
        r_t, c_t = pos_t
        if maze[r_t][c_t] != PLAYER: # Se o jogador ainda não pegou, ele está lá
            maze[r_t][c_t] = TREASURE
            
        print_maze(maze, step_count, "Em jogo. J moveu para o Tesouro. E moveu para J.")
        time.sleep(DELAI_PASSO)


# Executa o Jogo
if __name__ == "__main__":
    try:
        main_game_loop()
    except KeyboardInterrupt:
        print("\nJogo encerrado pelo usuário.")